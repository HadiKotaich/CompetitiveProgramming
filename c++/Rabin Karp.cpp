struct RabinKarp { // Rabin Karp
    ...
    // returns the hash s[0 ... l] + s[r ... n]
    array<ll, 2> query_preSuff(const int l, const int r, const int n) {
        array<ll, 2> res = { 0, 0 };
        for (int z = 0; z < modSz; ++z) {
            ll left = h[l + 1][z];
            ll right = (h[n + 1][z] - h[r][z] * pPow[n + 1 - r][z]) % mod[z];
            res[z] = (right + left * pPow[n + 1 - r][z]) % mod[z];
            if (res[z] < 0)
                res[z] += mod[z];
        }
        return res;
    ...
    // returns the hash s[0 ... l] + s[r ... n]
    ll queryLL_preSuff(const int l, const int r, const int n) {
        if (r > n) {
            return queryLL(0, l);
        }

        auto a = query_preSuff(l, r, n);
        return ((a[0] << 32) + a[1]);
    }
};
/*
